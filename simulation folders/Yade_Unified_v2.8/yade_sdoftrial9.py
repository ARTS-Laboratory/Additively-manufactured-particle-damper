...
from common_functions import ball_gen_8_output_filename, calc_PLL_gain, wall_generate
...
if ( len(sys.argv) <= 3):
        ...
        if ( sys.argv[3] == 'batch'):
                ...
                timing = False
        # SAMUEL ADDED CODE
        # Added functionality for indent here as we never use anything other than the 'interactive' mode
        elif ( sys.argv[3] == 'indent'):
            batch = False
            timing = False
            indent = True
        else:
...
# SAMUEL CHANGED CODE
# Commented out the wall_generate function to replace with the one from common functions
bc = periodic_bc
...
if (not restart):
  ...
  for i in range(n_windows_per_pocket):
          ...
          # SAMUEL ADDED CODE
          # This generates the wall sphere used for the indent. It pulls information generated from ball_gen_8_and_15.py automatically
          # Wall sphere density is set low in case it would negatively affect calculations
          # Wall sphere density can be set low because it gets clumped immediately and thus won't be forced away by higher density particles.
          # -----------------
          if indent:
              with open('Indent_Loc.csv', 'r') as indentFile:
                  indentReader = csv.DictReader(indentFile, skipinitialspace=True)
                  indentRowList = list(indentReader)
                  indentX = float( indentRowList[0]['indentX'])
                  indentY = float( indentRowList[0]['indentY'])
                  indentZ = float( indentRowList[0]['indentZ'])
                  indentRad = float( indentRowList[0]['indentRad'])


              O.materials.append(FrictMat(young=young_wl, poisson=0.3, frictionAngle=finalFricDegree, density=0.001, label='massless'))
              # This material should have zero mass but it causes a divide by zero error, so we'll assume 0.001 is low enough to not greatly impact calculations
              wallSphere = yade.utils.sphere(
                    (indentX, indentY, indentZ),
                    indentRad, material='massless', wire=True)


              wallSphereId = O.bodies.append(wallSphere)
              O.bodies.addToClump([wallSphereId], wallClump)
...
ball_ke_output= True #turn these off to reduce file size and slight increase to computation time
...
if (not restart):                
        ...
        O.engines=[
                ...
                PyRunner(command='get_KE()', realPeriod=1),
                PyRunner(command='get_initial_data()', iterPeriod=1, nDo=1, firstIterRun=1),
...
def get_initial_data():
    print('q1 initial force'+str(O.forces.f(q1_id)[1]))
...
def swing_coupled_extF_new():
        ...
        if (force_method == 2) or (force_method == 1):
                base_vel = 0
        else:
...
#        print(base_vel)
        ...
        for i in range( len( wall_state_list)):  #loop overs the windows
                ...
                if (indent == True) and (sidewall_frict_on == 0):
                    wallF1 = ( O.forces.f(topWall_list[i] )[1] +  O.forces.f(botWall_list[i] )[1] + O.forces.f(wallSphereId)[1]) * balls_mass_ratio * Psi_pocket_list[i]
                elif sidewall_frict_on == 0:
                    ...
                    #print('topWallF: '+str(O.forces.f(topWall_list[i])[1])+' + botWallF: '+str(O.forces.f(botWall_list[i])[1])+' * balls_mass_ratio: '+str(balls_mass_ratio)+' * Psi_pocket_list['+str(i)+']: '+str(Psi_pocket_list[i]))
                else:
        ...
        if O.iter == 1:
            O.forces.addF(q1_id, (0, 1000 + extF + kF + wallF + cF - grav_y * Modal_Mass, 0))
        else:
            O.forces.addF(q1_id , ( 0, 1000 + extF + kF + wallF + cF - grav_y * Modal_Mass , 0) )

        #print('extF: '+str(extF)+' + kF: '+str(kF)+' + wallF:'+str(wallF)+' + cF: '+str(cF)+' - grav_y: '+str(grav_y)+' * ModalMass: '+str(Modal_Mass))
...
# SAMUEL ADDED CODE
# This is the graphing function from ball_gen_8_and_15.py
# Was used early in our simulations when we didn't know what data to look at, but has been modified to be more useful to us
testing = True
def addPlotData():
    if (not batch):
            plot.addData(time=O.time, displacement=q1_state.pos[1] + base_displacement_amp * sin(theta),
                         #bot_force_y=np.log10(np.abs(O.forces.f(botWallId)[1])),
                         #net_force_x=np.log10(np.abs(O.forces.f(wallClump)[1])),
                         #right_force_x=np.log10(np.abs(O.forces.f(rightWallId)[0])),
                         time2=O.time,
                         KE=np.log10(currentKE))
            # print(q1_state.pos[1] + base_displacement_amp * sin(theta))
            # print(currentKE)

if (not batch):

        plot.plots={'time':('displacement'), 'time2':('KE') }
